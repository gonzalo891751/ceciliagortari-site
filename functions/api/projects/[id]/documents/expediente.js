...
export async function onRequestPost(context) {
  ...
  try {
    ...
    // Only PDF allowed for expediente
    const ext = getExtension(file.name);
    if (ext !== ".pdf" && file.type !== "application/pdf") {
      return new Response(
        JSON.stringify({ ok: false, error: "El expediente debe ser un archivo PDF." }),
        { status: 400, headers: HEADERS }
      );
    }

    // Delete existing expediente if any (replacement)
    const existing = await db
      .prepare(
        "SELECT id, stored_key FROM project_documents WHERE project_id = ? AND kind = 'expediente'"
      )
      .bind(projectId)
      .first();
    ...
    const storedKey = `projects/${projectId}/expediente/${timestamp}_${safeName}`;
...
export async function onRequestDelete(context) {
  ...
  try {
    ...
    if (!existing) {
      return new Response(
        JSON.stringify({ ok: false, error: "No hay expediente cargado." }),
